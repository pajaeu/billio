            ...
            <div class="pe-6 md:pe-12 p-2">
                <div class="relative" x-data="{ show: false }" @click.outside.window.stop="show = false">
                    ...
                    <div class="absolute z-10 right-0 top-full p-2 w-max bg-white rounded shadow" x-show="show">
                        ...
                        <button wire:click="copy({{ $invoice->id }})" wire:confirm="Opravdu chcete zkopírovat tuto fakturu?" class="cursor-pointer w-full py-2 px-4 flex items-center gap-2 text-slate-500 hover:text-slate-800 rounded hover:bg-slate-100 transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
